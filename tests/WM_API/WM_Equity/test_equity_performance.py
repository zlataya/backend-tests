...
def test_equity_performance(p_name, p_id, from_dt, to_dt, p_dt, wm_api, expect):
    ...
    for dt in performance_in:
        if dt in performance_out:
            ...
            expect(pytest.approx(p_in, abs=0.001) == p_out,
                   'Fail: Equity performance: %s: api %s != %s db' % (dt, p_out, p_in))
        else:
            expect(dt in performance_out, 'Fail: Equity performance is missing for date: %s' % dt)
