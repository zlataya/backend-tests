...
def test_dividends(p_name, p_id, tense, from_dt, to_dt, wm_api, expect):
    ...
    # get the BE data and pack it into dict
    api_data = wm_api['equity'].post(json.dumps(body), url_param='.dividends')
    ...
    for dt in income_in:
        ...
        expect(i_in == i_out, 'Fail: Equity dividends: for date %s: api %s != %s db' % (dt, i_out, i_in))
